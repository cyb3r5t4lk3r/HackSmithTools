...
Skript provádí rekurzivní průchod webem v rámci zadaného rozsahu a hledá JavaScript soubory. U každého nalezeného JavaScript souboru se pokusí najít odpovídající sourcemap soubor.
...
6. Všechny nalezené JavaScript soubory deduplikuje.
7. Pro každý JavaScript soubor zkusí najít odpovídající sourcemap.
8. Pokud sourcemap existuje, analyzuje její základní strukturu.
9. Volitelně extrahuje obsah `sourcesContent` do lokální složky.
10. Výsledek zapíše do konzole nebo do JSON reportu.

Ve výchozím nastavení skript zůstává v rámci stejného originu, tedy například při spuštění proti `https://reactjs.org` nebude automaticky procházet cizí domény třetích stran.
...
| `--all-origins` | Volitelně povolí zpracování odkazů mimo původní origin. Používat opatrně. |
